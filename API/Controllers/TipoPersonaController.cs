...
public class TipoPersonaController : BaseApiController
{
    ...
    {
        var appointments = await _unitOfWork.TipoPersonas
                                    .GetAllAsync();

        return _mapper.Map<List<TipoPersonaDto>>(appointments);
    }
    ...
    {
        var appointment = await _unitOfWork.TipoPersonas.GetByIdAsync(id);
        return _mapper.Map<TipoPersonaDto>(appointment);
    }
